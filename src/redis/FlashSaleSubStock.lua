...
-- 秒杀前缀 field
local act_flash_sale_pre ="act_flash_sale_{product}:";
...
local subStock = function()
    for i, v in ipairs(ARGV) do
        ...
        elseif aclPStock < 0 then
            ...
            callBackTab[i]=el;
            error(getResponseInfo(UNDER_STOCK_CODE,"秒杀id:".. actFlashSaleProductId .. "剩余库存不足"));
        else
            el.actFlashSaleProductId = actFlashSaleProductId;
            el.stockNum = stockNum;
            callBackTab[i]=el;
...
local plusStock = function()
    for i, v in ipairs(callBackTab) do
        ...
        -- 秒杀ID 不为 nil 加库存
        if actFlashSaleProductId ~= nil then
            redis.pcall("INCRBY", act_flash_sale_key, stockNum);
        end
